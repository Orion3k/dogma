    ...
    for j in range(grid_cell_array.get_length()):
        if grid_cell_array.get_cell_attr(j, "start_index") != None:
            ...
            if (m_occ_pred > particle_array.p_S):
                ...
                particle_array.normalize_p_S(grid_cell_array.get_cell_attr(j, "start_index"),
                                             grid_cell_array.get_cell_attr(j, "end_index"))
            ...
            m_occ_up, m_free_up = update_of(m_occ_pred, m_free_pred, meas_cell_array.get_cell_attr(j, "m_occ"),
                                            meas_cell_array.get_cell_attr(j, "m_free"))
            ...
            if check_values and (m_occ_up > 1 or m_occ_up < 0):
                print("start index: ", grid_cell_array.get_cell_attr(j, "start_index"), "end index: ",
                      grid_cell_array.get_cell_attr(j, "end_index"), "mass_occ: ", m_occ_up, "mass_free: ", m_free_up, "weight[start]: ",
                      weight_array_accum[grid_cell_array.get_cell_attr(j, "start_index") - 1], "weight[end]: ", weight_array_accum[grid_cell_array.get_cell_attr(j, "end_index")])
            ...
            if check_values: assert (
                all([rho_b >= 0., rho_p >= 0., m_free_up >= 0., m_occ_up >= 0., m_occ_up <= 1., m_free_up <= 1.]))
