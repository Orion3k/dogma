...
def PersistentParticleUpdate(particle_array, grid_cell_array, meas_cell_array, check_values=False):
    ...
    for i in range(particle_array.get_length()):
        weight_array.append(
            update_unnorm(particle_array.get_weight(i), particle_array.get_cell_idx(i), meas_cell_array))
    ...
    if check_values:
        ...
        if (start_index != None and end_index != 1):
            print("Sum of particles in the grid cell: ",
                  weight_array_accum[end_index] - weight_array_accum[start_index - 1])
